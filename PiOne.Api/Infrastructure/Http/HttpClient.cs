...
namespace PiOne.Api.Infrastructure.Http
{
    ...
    /// It uses IAuthenticator or ICertificationAuthenticator to do the signing
    /// </summary>
    internal class HttpClient
    {
        ...
        public HttpClient(string baseUri, IConsumer consumer, IUser user)
            : this(baseUri)
        {
            User = user;
            Consumer = consumer;
        ...
        public Response Delete(string endpoint)
        {
            ...
            {
                if (we.Response != null) { return new Response((HttpWebResponse)we.Response); }
                throw;
            }
        }
        private HttpWebRequest CreateRequest(string endPoint, string method, string accept = "application/json", string query = null)
        {
            ...
            AddHeaders(request);
            request.UserAgent = !string.IsNullOrWhiteSpace(UserAgent) ? UserAgent : string.Format("Newstead Api wrapper - ", Consumer.ConsumerKey);
        ...
        private void WriteMultipartData(byte[] bytes, HttpWebRequest request, string contentType, string name, string filename)
        {
            ...
            var t = "\r\n--" + boundary + "--\r\n";
            byte[] header = Encoding.ASCII.GetBytes(h);
            byte[] trailer = Encoding.ASCII.GetBytes(t);
